...
2. The corrected mapping in `packages/forms/src/samples/forms.ts`, per the spec's table. Note that Council stays `null`, unchanged from today, and `Location.address.city` stays unmapped. See the constraint below before writing that snapshot.
...
**Constraint: Council keeps no Address slot, and `address.city` is reserved for Ward.** `facility_registry` carries six administrative columns, `zone, region, district, council, ward, village` (`packages/db/src/facility-answers.ts:24`), and the source hierarchy is Zone, Region, District, Council, Ward, Village (`docs/superpowers/specs/2026-08-04-facility-registry-design.md:49`). Only the first four are in `FACILITY_ADMIN_LEVELS` and on the form. Ward and Village are the populated places `address.city` describes; a Council is a local government authority, not a settlement. So Council maps to `null`, the same as Zone, and `Location.address.city` is left unmapped rather than filled with the nearest thing to hand.

This matters for `facility-admin-order`. Two of the four form tiers now carry no path, so the rule compares only the tiers that do have one, and it must treat a `null` path as "skip this level", not as a violation. A rule that required all four levels to be bound would fail the corrected form it was written to protect.

Two traps in the general literature, both of which would produce a wrong snapshot:

- Guidance on Tanzanian administrative geography routinely glosses ADM2 as "District or Council" and treats them as synonyms. Here they are adjacent distinct tiers, and the import mapper keeps them apart (`docs/superpowers/specs/2026-08-12-facility-import-mapping-design.md:78`). Following the gloss collides Council and District on `address.district`.
- The ISO 21090 ADXP address-part extensions do not rescue Zone or Council. `precinct`, `county`, and `censusTract` all sit below `city` and `state`, the wrong direction for a supra-regional tier.
