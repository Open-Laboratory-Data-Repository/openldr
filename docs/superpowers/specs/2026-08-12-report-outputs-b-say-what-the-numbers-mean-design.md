...
| The PDF is the only consumer of the GLASS query | **False, and this is the constraint that shapes the design.** `r-amr-glass-ris` also feeds `GET /api/reports/glass/ris.csv` (`apps/server/src/reports-routes.ts:65-70`) and `openldr report glass-export --out` (`packages/cli/src/report.ts:60-73`). Both call `toCsv(result.columns, result.rows)`, so **any column added to the query silently changes a file submitted to a national programme.** |
