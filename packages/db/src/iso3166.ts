/**
 * ISO 3166-1 countries — alpha-3 code + the ISO official English short name.
 *
 * GENERATED from packages/db/fixtures/iso3166/iso3166-1.csv (public domain; see that
 * directory's README for provenance). Inlined rather than parsed at import time because
 * migration 073 needs a frozen snapshot and @openldr/db must not read a CSV to be imported.
 *
 * ⛔ Do NOT hand-edit. Regenerate from the fixture and let iso3166.test.ts prove the two
 * still agree — that drift guard is the whole point of keeping both.
 *
 * ⚠ Six names carry diacritics (Åland Islands, Côte d'Ivoire, Curaçao, Réunion,
 * Saint Barthélemy, Türkiye). This file is UTF-8; keep it that way.
 *
 * ⚠ Names are ISO's official English short names, so they read formally
